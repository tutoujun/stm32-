...
u8 NRF_TX_DATA[TX_PLOAD_WIDTH]={0x55,0x55,0x55,0x55,0x55,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
...
{
    CE_LOW();
  	Nrf24l01_WriteBuf(W_ACK_PAYLOAD,txbuf,len);//写数据到TX BUF  32个字节
 	CE_HIGH();  
}
/*****************************************************************************
...
void nRF24l01_RxCONFIG(void){
	CE_LOW();
	Nrf24l01_WriteReg(Write_Reg+FEATURE,      0x06);                //使能动态负载(收发数据)长度
    Nrf24l01_WriteReg(Write_Reg+DYNPD,        0x01);                //使能通道0动态负载长度
...
void EXTI2_IRQHandler(void){
	if(EXTI_GetITStatus(EXTI_Line2)==SET){
		u8 sta = Nrf24l01_ReadReg(StATUS);
		if(sta & TX_OK)
		{
            CE_LOW();
			Nrf24l01_WriteReg(Write_Reg + StATUS,TX_OK); //清除发送完成标志·
			Nrf24l01_WriteReg(Flush_Tx,0xff); //清除TX_FIFO
            //NRF_TX_OK = 1;
            CE_HIGH();
        }
        ...
        {
            CE_LOW();
            u8 len = Nrf24l01_ReadReg(R_RX_PL_WID);
            if(len>0 && len<33)//数据长度有效
            {
                Nrf24l01_ReadBuf(RD_RX_Pload,RxBuf,32);
            }
			nrf_txPacket_AP(NRF_TX_DATA,32);//发送ACK Payload,数据长度为1
			Nrf24l01_WriteReg(Write_Reg + StATUS,RX_OK);//清除接收完成标志
        ...
        if(sta & MAX)
		{
            CE_LOW();
            //NRF_TX_ERROR = 1;
			Nrf24l01_WriteReg(Write_Reg + StATUS,MAX);//清除达到最大重发标志
			Nrf24l01_WriteReg(Flush_Tx,0xff); //清除TX_FIFO
            CE_HIGH();
        }
